...
export default function CheckoutModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg bg-[#0D1730] border border-[#00C6FF]/30 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-black/80 overflow-hidden">
        {/* Subtle Ambient Glow */}
        <div className="absolute -top-24 -right-24 w-48 h-48 bg-[#0066FF]/25 rounded-full blur-3xl pointer-events-none" />

        {/* Modal Header */}
        <div className="flex items-center justify-between pb-5 border-b border-white/10">
          <div>
            <div className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-[#00C6FF]">
        ...
        {/* Plan Summary Banner */}
        <div className="mt-5 p-4 rounded-2xl bg-[#050B18]/70 border border-[#00C6FF]/20 flex items-center justify-between">
          <div>
            ...
            <span className="text-xs text-[#00C6FF] font-semibold">{selectedPlan.durationLabel} Access</span>
          </div>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-xl bg-[#050B18] border border-white/15 text-white placeholder-zinc-500 text-sm focus:outline-none focus:border-[#00C6FF] focus:ring-1 focus:ring-[#00C6FF] transition-colors"
            />
          </div>
          ...
          {/* Demo Sandbox Note */}
          <div className="p-3 rounded-xl bg-[#0066FF]/15 border border-[#00C6FF]/30 text-zinc-300 text-[11px] leading-relaxed flex items-start gap-2">
            <ShieldCheck className="w-4 h-4 shrink-0 text-[#00C6FF] mt-0.5" />
            <span>
              <strong className="text-[#00C6FF]">Demo Gateway Active:</strong> Instant simulated server authorization for client presentation. No actual card charge occurs.
            </span>
          </div>
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full mt-2 py-4 rounded-full bg-gradient-to-r from-[#0066FF] to-[#00C6FF] hover:brightness-110 text-white font-heading font-black text-sm uppercase tracking-wider flex items-center justify-center gap-2 shadow-lg shadow-[#0066FF]/40 hover:shadow-[#00C6FF]/60 transition-all transform hover:-translate-y-0.5 active:scale-[0.98] disabled:opacity-50 cursor-pointer"
          >
            {isLoading ? (
              <>
